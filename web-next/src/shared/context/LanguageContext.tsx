...
import { lookupTranslation } from "@/shared/context/translations";
...
async function detectCountry(): Promise<string | null> {
  // Goes through our own BFF (/api/geo) — same-origin, so no CORS. The Next
  // server does the IP geolocation using the real client IP server-side.
  try {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 5000);
    const response = await fetch(joinUrl(API_BASE_URL, "/geo"), { signal: controller.signal });
    clearTimeout(timer);
    if (!response.ok) return null;
    ...
    // Ignore — geo detection is best-effort; browser-language fallback remains.
  }
  return null;
}
