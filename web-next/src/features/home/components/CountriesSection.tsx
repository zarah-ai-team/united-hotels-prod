...
import { hotels } from "@/shared/data/mockData";
import {
  MapPin,
  Building2,
  TrendingDown,
...
function HotelCard({ hotel }: HotelCardProps) {
  ...
  return (
    <Link
      to="/listing"
      className="group block p-4 bg-white border border-[#e5e7eb] rounded-xl hover:border-[#2F80ED] hover:shadow-lg transition-all duration-300"
    >
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="font-['Inter:Bold',sans-serif] text-[14px] text-[#2F80ED]">
              {formatCurrency(hotel.rooms[0]?.directPrice, language)}/night
            </span>
          </div>
        </div>
        <ChevronRight className="w-5 h-5 text-[#2F80ED] group-hover:translate-x-1 transition-transform" />
      </div>
    </Link>
  );
}
...
export function CountriesSection() {
  return (
    <section className="py-10 md:py-24 bg-[#fafafa] relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#2F80ED]/3 rounded-full blur-3xl translate-x-1/3 -translate-y-1/3" />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-[#2F80ED]/3 rounded-full blur-3xl -translate-x-1/3 translate-y-1/3" />

      <div className="max-w-[1840px] mx-auto px-10 relative">
        {/* Section Header */}
        <div className="text-center mb-16 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#2F80ED]/10 text-[#2F80ED] rounded-full font-['Inter:SemiBold',sans-serif] text-[13px] tracking-wide uppercase mb-6">
          ...
          <div className="flex items-center justify-center gap-2 mt-6">
            <div className="w-16 h-1 bg-gradient-to-r from-transparent to-[#2F80ED] rounded-full" />
            <div className="w-2 h-2 bg-[#2F80ED] rounded-full" />
            <div className="w-16 h-1 bg-gradient-to-l from-transparent to-[#2F80ED] rounded-full" />
          </div>
        </div>
        ...
        {/* Bottom Info Bar */}
        <div className="bg-gradient-to-r from-[#2F80ED]/5 via-[#2F80ED]/10 to-[#2F80ED]/5 rounded-2xl p-6 sm:p-8 mt-12 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-400">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 lg:gap-0">
            {/* Left Side */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              <div className="bg-[#2F80ED]/10 p-4 rounded-xl">
            ...
            <Link
              ...
              className="group inline-flex items-center gap-2 border-2 border-[#2F80ED] text-[#2F80ED] px-8 py-3 rounded-xl hover:bg-[#2F80ED] hover:text-white transition-all duration-300 font-['Inter:Bold',sans-serif] text-[15px] whitespace-nowrap"
            >
              View All Hotels
