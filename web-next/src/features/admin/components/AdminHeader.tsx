        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="relative">
            ...
            >
              ...
              <span className="absolute right-1 top-1 h-1.5 w-1.5 rounded-full bg-[#EF4444] ring-2 ring-white dark:ring-[#141821]" />
            </button>

            {showNotifications && (
              <div
                className="absolute right-0 mt-2 w-72 bg-white/95 dark:bg-[rgba(20,24,33,0.95)] rounded-xl shadow-xl border border-[#EAEAEA] dark:border-white/10 py-1 z-50"
              ...
              >
                ...
                <div className="max-h-80 overflow-y-auto">
                  {notifications.map((notif) => (
                    <button
                      key={notif.id}
                      className={`w-full px-3 py-2 text-left hover:bg-[#FAFAFA] dark:hover:bg-white/[0.06] transition-colors border-b border-[#EAEAEA]/60 dark:border-white/[0.06] last:border-0 ${
                        notif.unread ? 'bg-[#2F80ED]/[0.04] dark:bg-[#2F80ED]/[0.12]' : ''
                      }`}
                    >
                      <p className="text-[12.5px] text-[#3B3B3B] dark:text-white/85 mb-0.5 leading-snug" style={{ fontFamily: 'Inter, sans-serif' }}>
                        {notif.text}
                      </p>
                      <p className="text-[10.5px] text-[#9aa0a6] dark:text-white/45">{notif.time}</p>
                    </button>
                  ))}
                </div>
                <div className="px-3 py-2 border-t border-[#EAEAEA] dark:border-white/10">
                  <button className="text-[12px] text-[#2F80ED] dark:text-[#5DA0F8] font-medium hover:text-[#1E5FBC] dark:hover:text-[#7DB4FA]">
