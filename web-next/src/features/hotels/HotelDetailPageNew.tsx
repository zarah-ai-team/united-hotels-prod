...
import { HotelDetailLoader } from "@/shared/components/HotelLoadingState";
import { useBooking } from "@/shared/context/BookingContext";
import { hotelService, type PublicHotel, type PublicHotelRoom } from "@/shared/api/services";
...
function HotelPolicies({ childPolicy, petPolicy, smokingPolicy, cancellationPolicy }: HotelPoliciesProps) {
  const { t } = useLanguage();
  type Policy = {
  ...
  const policies: Policy[] = [
    ...
    cancellationPolicy && ({
      icon: <CalendarClock className="w-[15px] h-[15px]" strokeWidth={1.9} />,
...
function HotelContactCard({ contactName, contactPhone, email }: HotelContactCardProps) {
  ...
  const supportRows: Row[] = [
    ...
    { icon: <MessageCircle className="w-[15px] h-[15px]" strokeWidth={1.9} />, label: "WhatsApp Support", value: supportNumber, href: "https://wa.me/905449580798", sub: "Reply within minutes", accent: "teal", external: true },
...
export function HotelDetailPageNew() {
  ...
  const cancellationPolicy = hotel.cancellation_policy || hotel.cancellationPolicy;
      ...
      <section className="glass-section-bg pt-6 md:pt-10 pb-20 md:pb-28 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_380px] gap-6 items-start">
          ...
          <div>
            ...
            {/* 6. Policies */}
            {(childPolicy || petPolicy || smokingPolicy || cancellationPolicy) && (
              <Section title="Hotel Policies" icon={<CheckCircle className="w-5 h-5" />}>
                <HotelPolicies childPolicy={childPolicy} petPolicy={petPolicy} smokingPolicy={smokingPolicy} cancellationPolicy={cancellationPolicy} />
              </Section>
            )}
